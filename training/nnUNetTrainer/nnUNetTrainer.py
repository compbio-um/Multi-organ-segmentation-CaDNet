...
class nnUNetTrainer(object):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # model output (juneli-20251127)
            output, organ_pred, discrim_tumor, discrim_latent = self.network(data)
            
            l = self.loss(output, organ_pred, discrim_tumor, discrim_latent, target)
            del data
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.enable_deep_supervision:
            output = output[0]
            target = target[0]
            ## add organ loss（JuneLi, 20250725）
            organ_pred = organ_pred[0]
