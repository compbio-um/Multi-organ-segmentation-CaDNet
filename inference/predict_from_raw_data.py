...
#from nnunetv2.training.nnUNetTrainer.create_model import create_model
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        #network = create_model(name = 'DNet2D')
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Network output (Juneli,20251121)
        #prediction, _, _, _ = self.network(x)
        #print('######### input X (mirror1): %s ##########'%str(prediction.shape))

        # Network causal output (Juneli,20260422)
        _, prediction, _, _ = self.network(x)
        #print('######### input X (mirror1): %s ##########'%str(prediction.shape))
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                #print('######### input X (mirror2): %s ##########'%str((self.network(torch.flip(x, axes))[0]).shape))
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    if item == 'end':
                        ...
                        break
                    workon, sl = item
                    #print('######### final output (workon): %s #############' % (str(self._internal_maybe_mirror_and_predict(workon)[0].shape)))
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset003_Liver/nnUNetTrainer__nnUNetPlans__2d/'),
        use_folds=(4,),
    ...
    '''ret = predictor.predict_from_files_sequential(
        [['/home/juneli/nnUNet/nnUNet/nnUNetFrame/DATASET/nnUNet_raw/Dataset113_Pancreas_tumor/imagesTr/pancreas_002_0000.nii.gz'], \
         ['/home/juneli/nnUNet/nnUNet/nnUNetFrame/DATASET/nnUNet_raw/Dataset113_Pancreas_tumor/imagesTr/pancreas_006_0000.nii.gz']],
        '/home/juneli/nnUNet/nnUNet/nnUNetFrame/DATASET/nnUNet_output/DUNet_test', False, True, None
    )'''

    val_file = '/home/juneli/nnUNet/nnUNet/nnUNetFrame/DATASET/nnUNet_raw/Dataset003_Liver/imagesTs/'
    files_all = []
    for file_i in os.listdir(val_file):
        #print(file_i)
        path_all = val_file + file_i
        files_all.append([path_all])
    ...
    ret = predictor.predict_from_files_sequential(
        files_all,
        '/home/juneli/nnUNet/nnUNet/nnUNetFrame/DATASET/nnUNet_output/1.MSD/liver/Ablation/fold4_1638441/', False, True, None
    )
